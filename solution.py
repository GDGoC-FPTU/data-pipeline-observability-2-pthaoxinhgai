...
def validate(data):
    ...
    for record in data:
        try:
            ...
            error_count += 1
            print(f"Dropped record id={record.get('id')}: invalid price ({record.get('price')})")
            continue
        ...
        cleaned_record['price'] = int(price) if price.is_integer() else price
        cleaned_record['category'] = str(category).strip()
        valid_records.append(cleaned_record)

    print(f"Validation summary: {len(valid_records)} valid records, {error_count} errors dropped.")
